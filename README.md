...
👉 This tutorial is Android-focused, but you will be able to use test-ai-appium with iOS as well.
...
In the following tutorial, you will learn about how to setup and run AI-enhanced appium with test.ai.
### Setup and Run
...
Now we'll install Java the dependencies.  `cd` into the root directory of this repository and run 
```bash
./gradlew build
```
...
1. Open Android Studio and create a new project.
   ...
   ⚠️ if the [Setup Wizard](https://developer.android.com/studio/install) appears, please complete this and install all the additional dependencies it asks you to.
...
3. Click `Next` and choose the option for `Pie`.
   ...
   👉 If there is a `download` button next to `Pie`, please click it, wait for the download to complete, then click `Finish`. 
...
👉 It is advisable to launch Appium in its own terminal window since it does not yield terminal control back to you after starting.
...
This sample code is using the standard Appium selectors without any AI, which is how most apps today are tested.  However, these can be fragile and break easily, as even small changes to an App can cause these selectors to *immediately* stop working.  Fortunately, the test.ai SDK is equipped to help you avoid this unecessary hassle.

### Using test.ai with Appium

Please visit https://sdk.test.ai (and log in to your test.ai account if you've been logged out).

You should see the following entries on this page:
...
Believe it or not, you just used AI!  test.ai is visual-based, so there's no need to mess around with Appium selectors.  The test.ai classifier will train itself using the element contained in the box you just drew with your mouse, and now, when it encounters this element in the future, it'll be able to recognize it!  

Now, let's do the same thing for the other elements.
...
👉 Training takes a few minutes, you can training status by visiting https://sdk.test.ai/training_status
...
* The `AndroidDriver` gets passed as a parameter to a `TestAiDriver`.
...
As you may have already gathered, a standard Appium test would certainly fail, but test.ai won't!

⚠️ If you closed Appium and/or the Android Emulator, please restart them with the instructions in the section above.
